...
with tempfile.TemporaryDirectory(prefix="pgfy-e2e-") as temporary:
    ...
    env = dict(os.environ, PGFY_CONFIG=str(directory / "install.json"), PGFY_DB=str(directory / "metadata.db"), PGFY_KEY_FILE=str(directory / "key"), PGFY_HEALTH_PASSWORD_FILE=str(directory / "absent"), PGFY_LISTEN="127.0.0.1:3000" if args.backend_only else "127.0.0.1:8080")
    ...
    token_file = root / ".cache/e2e-token"
