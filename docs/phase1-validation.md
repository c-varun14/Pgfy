...
- **Capacity on the $12 instance (`pgfy-b`, 2 vCPU / 2 GiB, Xeon Platinum 8259CL), PostgreSQL 18.6 in a throwaway container with `max_connections=200`, pgbench client on the same host:** 50 databases (pgbench scale 2 each, 1.9 GB on disk) idle at 227 MiB; with 100 idle client connections (two per database) 508 MiB. Ten databases loaded simultaneously with 5 clients each: 1,431 mixed tx/s; ten databases read-only: 6,942 reads/s; all 50 databases writing at once with one client each: 1,252 tx/s. Single 755 MB database (5 M rows), 20 clients: 15,781 select-only reads/s at 1.3 ms, 1,933 mixed tx/s at 10 ms. `pg_dump --format=custom` of that database: 6.1 s (15 MB archive); `pg_restore` into a new database: 27.9 s; row-count verification: 4.8 s. Burstable vCPUs: sustained load beyond a few minutes will draw down CPU credits; these are 20–30 s runs.
