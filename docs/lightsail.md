...
3. In Lightsail's IPv4 firewall, allow TCP 80/443 for the HTTPS dashboard and TCP 5432 for application database connections (restrict its source to your application's addresses when you can; the per-project allowlist in the dashboard is the second layer). Restrict TCP 22 to operator source addresses. Configure IPv6 rules separately if enabled. Leave application 3000, Caddy admin 2019, and tunnel/internal health 8080/8081 closed.
...
Lightsail's firewall controls public-address traffic; it does not replace private-network access controls. PostgreSQL's own policy (TLS-only, per-project allowlists, SCRAM passwords) applies on every path, including the private network. Probe from a private-network peer as well as an external client. If host firewall rules are used, account for Docker's actual forwarding behavior.
...
Retain the static-IP association, SSH access, installation identity, release version, and separately protected configuration/secrets in the operator's records. A static IP is not a data backup.
