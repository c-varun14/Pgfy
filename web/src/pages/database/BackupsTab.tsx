...
import { api, type BackupHistory, type BackupRecord, type Job, type Project } from "../../api";
...
export type { BackupHistory as History } from "../../api";
...
export function jobLabel(job: Job) {
  if (job.state === "succeeded") {
    if (job.kind !== "restore") return "Backup completed";
    return job.result.verification === "verified" ? "Restore verified" : job.result.verification === "partial" ? "Restored — partly verified" : "Restored — not verified";
  }
...
export function BackupsTab({ project, navigate }: { project: Project; navigate: (to: string) => void }) {
  const [history, setHistory] = useState<BackupHistory | null>(null); const [error, setError] = useState(""); const [busy, setBusy] = useState(false); const [selected, setSelected] = useState<BackupRecord | null>(null); const { showToast } = useToast();
  async function load() { try { setHistory(await api<BackupHistory>(`/projects/${project.id}/backups`)); setError(""); } catch (failure) { setError((failure as Error).message); } }
  ...
  const interval = history?.target_interval_hours ?? 24;
  const intervalLabel = interval === 1 ? "Hourly" : interval === 24 ? "Daily" : `Every ${interval} hours`;
  // The age shown is what the bucket holds, so a backup deleted there stops counting.
  const newest = history?.newest_backup_at || 0;
  const stale = !!newest && Date.now() / 1000 - newest > interval * 3600 * 1.5;
  return <div className="tab-stack">
    <Card><div className="backup-status"><div>{history?.storage_configured === false ? <><strong>Backups are off</strong><span> — </span><button className="link" onClick={() => navigate("/backups")}>Set up storage →</button></> : newest ? <>{intervalLabel} backups <strong>on</strong> · newest recoverable {relativeTime(newest)}{stale ? " (older than the target)" : ""}{history?.next_scheduled_at ? ` · next ${relativeTime(history.next_scheduled_at)}` : ""}{history?.failures ? ` · ${history.failures} failed attempt${history.failures === 1 ? "" : "s"}, retrying` : ""}</> : history ? <>{intervalLabel} backups <strong>on</strong> · first backup within a few minutes{history.failures ? ` · ${history.failures} failed attempt${history.failures === 1 ? "" : "s"}, retrying` : ""}</> : "Checking backup status…"}</div><Tooltip text={history?.storage_configured === false ? "Set up backup storage first" : "Create a backup now"}><Button onClick={() => void backupNow()} loading={busy} disabled={!history?.storage_configured || active}><Archive size={15} />{active ? "Running…" : "Back up now"}</Button></Tooltip></div>{error && <ErrorNotice message={error} />}</Card>
