...
export function ConnectionUrl({ project, host, sslmode, credentials, revealed, busy, error, onReveal, onHide, onCopy }: { project: Project; host: string; sslmode: string; credentials: Credentials | null; revealed: boolean; busy: boolean; error: string; onReveal: () => void; onHide: () => void; onCopy: () => void }) {
  ...
  return <div className="connection-url" data-busy={busy || undefined}><CodeBlock value={value} label="Connection URL" copy={false} /><div className="connection-actions">{revealed ? <Button variant="secondary" size="sm" onClick={onHide}><EyeOff size={14} />Hide password</Button> : <Button variant="secondary" size="sm" loading={busy} onClick={onReveal}><Eye size={14} />Reveal</Button>}<Button size="sm" loading={busy} onClick={onCopy}>Copy URL</Button></div>{error && <ErrorNotice message={error} />}</div>;
}
