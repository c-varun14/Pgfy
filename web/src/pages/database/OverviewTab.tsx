...
import { api, type BackupRecord, type Credentials, type Project } from "../../api";
import { relativeTime } from "../../lib/format";
import { Card, CardHeader } from "../../components/ui/card";
import { ConnectionUrl } from "./ConnectionUrl";

type History = { backups: BackupRecord[]; storage_configured: boolean; next_scheduled_at: number };
export function OverviewTab({ project, host, sslmode, credentials, revealed, credentialBusy, credentialError, onReveal, onHide, onCopy, onTab }: { project: Project; host: string; sslmode: string; credentials: Credentials | null; revealed: boolean; credentialBusy: boolean; credentialError: string; onReveal: () => void; onHide: () => void; onCopy: () => void; onTab: (tab: string) => void }) {
  ...
  useEffect(() => { void api<History>(`/projects/${project.id}/backups`).then(setHistory).catch(() => setHistory(null)); }, [project.id]);
  const sessions = project.connections_now?.length || 0; const backup = history?.backups[0];
  return <div className="tab-stack"><Card><CardHeader title="Connection URL" /><ConnectionUrl project={project} host={host} sslmode={sslmode} credentials={credentials} revealed={revealed} busy={credentialBusy} error={credentialError} onReveal={onReveal} onHide={onHide} onCopy={onCopy} /></Card>
    <Card><CardHeader title="Checklist" /><div className="checklist">
      ...
      <button type="button" onClick={() => onTab("backups")}>{history?.storage_configured === false ? <TriangleAlert size={17} className="warn" /> : backup ? <Check size={17} className="ok" /> : <Circle size={17} />}<span><strong>{history?.storage_configured === false ? "Backups are off" : backup ? `Backed up ${relativeTime(backup.created_at)}` : history ? `First backup scheduled ${history.next_scheduled_at ? relativeTime(history.next_scheduled_at) : "within a few minutes"}` : "Checking backups…"}</strong>{history?.storage_configured === false && <small>Backups →</small>}</span></button>
    </div></Card>
  </div>;
}
