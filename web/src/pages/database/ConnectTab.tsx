...
export function ConnectTab({ project, access, credentials, revealed, credentialBusy, credentialError, onReveal, onHide, onCopy, onRotated }: { project: Project; access: DatabaseAccess; credentials: Credentials | null; revealed: boolean; credentialBusy: boolean; credentialError: string; onReveal: () => void; onHide: () => void; onCopy: () => void; onRotated: (next: Credentials) => void }) {
  ...
  return <div className="tab-stack">
    ...
    <RotateCard project={project} onRotated={onRotated} />
...
function RotateCard({ project, onRotated }: { project: Project; onRotated: (next: Credentials) => void }) {
  ...
  async function rotate() { setBusy(true); setError(""); try { const body = await api<{ credentials: Credentials }>(`/projects/${project.id}/credentials/rotate`, { method: "POST", body: "{}" }); setFresh(body.credentials); onRotated(body.credentials); } catch (failure) { setError((failure as Error).message); } finally { setBusy(false); } }
  function close() { setConfirming(false); setFresh(null); setError(""); }
  return <Card><CardHeader title="Password" aside={project.rotation_pending && <Pill tone="wait">Change in progress</Pill>} />
    ...
    <Dialog open={confirming} onClose={close} title={fresh ? "New password" : "Change the password?"}>
      <div className="dialog-form">{fresh ? <><Banner><Check size={17} />Changed. Every session using the old password was closed.</Banner><p>Update your app's connection settings now. The new connection URL:</p><CodeBlock value={fresh.url} /><div className="actions"><Button onClick={close}>Done</Button></div></> : <>
        ...
        <div className="actions"><Button onClick={() => void rotate()} loading={busy}>{busy ? "Changing…" : "Change password"}</Button><Button type="button" variant="ghost" onClick={close}>Cancel</Button></div></>}</div>
    </Dialog>
  </Card>;
}
