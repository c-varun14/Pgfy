...
export function RestoreDialog({ manifest, open, busy = false, onClose, navigate, onComplete }: { manifest: Manifest | null; open: boolean; busy?: boolean; onClose: () => void; navigate: (to: string) => void; onComplete?: () => void }) {
  ...
  // A restore that ran is reported for what it is: the database exists either
  // way, and "verified" is claimed only when every check could be made.
  const verification = job?.result.verification; const done = job?.state === "succeeded";
  const title = done ? (verification === "verified" ? "Restore verified" : verification === "partial" ? "Restored — partly verified" : "Restored — not verified") : failed ? "Restore did not complete" : job ? "Restoring database" : `Restore ${manifest.project_name}`;
  return <Dialog open={open} onClose={onClose} title={title}>
    {!job ? <><p>Restores this backup into a <strong>new</strong> database named below.</p><Field label="Database name" htmlFor="restore-name" hint={`Nothing existing changes. Changes made after ${formatDate(Date.parse(manifest.created_at) / 1000)} are not included.`}><input id="restore-name" value={name} onChange={(event) => setName(event.target.value)} maxLength={64} required /></Field>{error && <ErrorNotice message={error} />}<div className="actions dialog-actions"><Button loading={starting} disabled={busy || !name.trim()} onClick={() => void restore()}>{starting ? "Starting…" : busy ? "Another job is running" : "Restore"}</Button><Button variant="ghost" onClick={onClose}>Cancel</Button></div></> : <div className="restore-progress"><Pill tone={done ? (verification === "verified" ? "good" : "wait") : failed ? "bad" : "wait"}>{done ? (verification === "verified" ? "Restore verified" : verification === "partial" ? "Partly verified" : "Not verified") : failed ? "Failed" : STAGE_LABELS[job.stage] || job.stage}</Pill>{job.result.summary && <p>{job.result.summary}</p>}{running && <><Progress /><p>{STAGE_LABELS[job.stage] || job.stage} · {formatDuration(job.elapsed_seconds)} elapsed</p><p className="caption">You can close this — it continues on the server.</p></>}{job.result.checks && <ul className="check-steps">{job.result.checks.map((check) => <li key={check.name} className={check.ok ? "ok" : "bad"}>{check.ok ? <Check size={14} /> : <TriangleAlert size={14} />}{check.name}{check.detail && <small>{check.detail}</small>}</li>)}</ul>}{!!job.result.restore_errors && job.result.stderr && <details className="restore-errors"><summary>{job.result.restore_errors} error{job.result.restore_errors === 1 ? "" : "s"} from pg_restore</summary><pre>{job.result.stderr}</pre></details>}
      {failed && <><ErrorNotice message={job.error || "The restore did not complete."} /><p>Nothing existing was modified.</p></>}{error && <ErrorNotice message={error} />}{done && <Button onClick={() => navigate(`/projects/${projectID}`)}>Open database <ArrowRight size={15} /></Button>}</div>}
  </Dialog>;
}
