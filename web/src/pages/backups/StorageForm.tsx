...
export const EMPTY_STORAGE: StorageSettings = { endpoint: "https://s3.us-east-1.amazonaws.com", region: "us-east-1", bucket: "", prefix: "pgfy", access_key: "", secret_key: "", session_token: "", path_style: false };
export function StorageForm({ initial, configured, onSaved }: { initial?: StorageSettings; configured: boolean; onSaved: (settings: StorageSettings) => void }) {
  ...
  useEffect(() => { setForm({ ...EMPTY_STORAGE, ...initial }); setAdvanced(!!initial && (initial.path_style || !!initial.session_token || !initial.endpoint.includes("amazonaws.com"))); }, [initial]);
  ...
  return <form className="storage-form" onSubmit={(event) => void save(event)}>
    ...
    {steps && <ul className="check-steps">{steps.map((step) => <li key={step.name} className={step.ok ? "ok" : "bad"}>{step.ok ? <Check size={14} /> : <TriangleAlert size={14} />}{step.name}{step.error && <small>{step.error}</small>}</li>)}</ul>}
  </form>;
}
